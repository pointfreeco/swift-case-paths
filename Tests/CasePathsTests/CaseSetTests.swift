...
extension CaseSet {
  // NB: Parameter packs cannot yet express '(each Path).Root == Element', so this spells the key
  //     path type out rather than using the constrained 'CaseKeyPath' alias.
  public func require<each Path: CasePath>(
    _ keyPath: repeat any KeyPath<Element.AllCasePaths, each Path> & Sendable
